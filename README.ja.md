...
- **PSR-4 対応** – `composer.json` の PSR-4 設定（namespace あたり単一ディレクトリ）を読み込み、ファイルパスと PHP クラス名をマッピング
...
> **注意:** `--log` を指定して `--output` を省略すると、マージ結果がベースラインの JUnit ファイルを上書きします。`--output` は常に明示的に指定することを推奨します。
